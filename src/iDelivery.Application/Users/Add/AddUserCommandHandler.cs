...
public sealed class AddUserCommandHandler : IRequestHandler<AddUserCommand, Result<UserResponse>>
{
    ...
    public async Task<Result<UserResponse>> Handle(AddUserCommand request, CancellationToken cancellationToken)
    {
        ...
        if(exists)
            return Result.Fail<UserResponse>(new EmailAlreadyExistsError());
        ...
        if(user is null)
            return Result.Fail<UserResponse>(new OperationFailedError("The user could not be created"));

        var success = await _accountRepository.AddUserAsync(accountId, user, cancellationToken);
        if (!success)
            return Result.Fail<UserResponse>(new OperationFailedError("An error occurred while attempting to save the user to the database"));
